...
| 5.7 | Step 3, the date. A calendar button opens a picker under the row, and the date sits beside it in a field that can be typed into directly; Clear means no date. A rescan of a product that is **no longer in the pantry** arrives pre-filled from `defaultShelfLifeDays`. The restock path in 5.4 never reaches this step at all. | `ui/capture/` | §3 |
